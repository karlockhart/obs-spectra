...
static bool RegServers(bool reg)
{
	...
	if (reg) {
		return RegServer(CLSID_OBS_VirtualVideo, L"OBS-Spectra Virtual Camera", file);
	} else {
...
static bool RegFilters(bool reg)
{
	...
	if (reg) {
		...
		hr = fm->RegisterFilter(CLSID_OBS_VirtualVideo, L"OBS-Spectra Virtual Camera", &moniker,
